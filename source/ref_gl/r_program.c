	...
	float zNear, float zFar )
{
	...
	if( program->loc.LastModelViewProjectionMatrix >= 0 ) {
		qglUniformMatrix4fv( program->loc.LastModelViewProjectionMatrix, 1, GL_FALSE, lastModelViewProjectionMatrix );
	}
...
void RP_UpdateBonesUniforms( int elem, unsigned int numBones, dualquat_t *animDualQuat, dualquat_t *prevAnimDualQuat )
{
	...
	qglUniform4fv( program->loc.PrevDualQuats, numBones * 2, &prevAnimDualQuat[0][0] );
}
...
static void RP_GetUniformLocations( glsl_program_t *program )
{
	...
	program->loc.LastModelViewProjectionMatrix = qglGetUniformLocation( program->object, "u_LastModelViewProjectionMatrix" );
	...
	locMotionVectorTexture = qglGetUniformLocation( program->object, "u_MotionVectorTexture" );
	locNormalTexture = qglGetUniformLocation( program->object, "u_NormalTexture" );
	...
	program->loc.PrevDualQuats = qglGetUniformLocation( program->object, "u_PrevDualQuats" );
...
//		qglUniform1i( locEntityDecalTexture, 4 );
	...
	if( locMotionVectorTexture >= 0 )
		qglUniform1i( locMotionVectorTexture, 1 );

	if( locNormalTexture >= 0 )
		qglUniform1i( locNormalTexture, 2 );
}
